Ext.BLANK_IMAGE_URL = 'http://localhost/extplorer/scripts/extjs3/resources/images/default/s.gif';
...
function openActionDialog( caller, action ) {
	...
	if( dontNeedSelection[action] == null  && selectedRows.length < 1 ) {
		Ext.Msg.alert( 'Error(s)','You haven\'t selected any item(s).');
		return false;
	}
	...
	switch( action ) {
											...
											if( oResponse && oResponse.responseText ) {
												...
												try{ json = Ext.decode( oResponse.responseText );
													if( json.error && typeof json.error != 'xml' ) {
														Ext.Msg.alert( "Error(s)", json.error );
														dialog.destroy();
														return false;
													}
												} catch(e) {
													msgbox = Ext.Msg.alert( "Error(s)", "JSON Decode Error: " + e.message );
													msgbox.setIcon( Ext.MessageBox.ERROR );
													return false; 
												}
												if( action == "edit" ) {
												...
												msgbox = Ext.Msg.alert( "Error(s)", "Received an empty response");
												msgbox.setIcon( Ext.MessageBox.ERROR );
		...
		case 'delete':
			...
			Ext.Msg.confirm('Delete?', String.format("Are you sure you want to delete these {0} item(s)?", num ), deleteFiles);
			break;
		case 'extract':
			Ext.Msg.confirm('Extract Archive?', "Do you really want to extract this file? Here?<br />This will overwrite existing files when not used carefully!", extractArchive);
			break;
		case 'download':
			document.location = 'index.php?option=com_extplorer&action=download&item='+ encodeURIComponent(ext_itemgrid.getSelectionModel().getSelected().get('name')) + '&dir=' + encodeURIComponent( datastore.directory );
			break;
	}
}
...
Ext.msgBoxSlider = function(){
    ...
    return {
        msg : function(title, format){
           ...
           Ext.get('x-box-mc-inner' ).setStyle('background-image', 'url("http://localhost/extplorer/images/_accept.png")');
